...
function openLifeModal(editId) {
  ...
  if (existingPhoto && !_lifePhotoData) _lifePhotoData = existingPhoto;
...
function initStickerCanvas() {
  ...
  var img = new Image();
  img.onload = function () {
    var maxW = Math.min(cv.parentElement.clientWidth - 2, 600);
...
function renderStickerCanvas() {
  ...
  img.onload = function () {
    ...
    var fs = Math.max(24, Math.round(cv.width / 15));
    _lifeStickers.forEach(function (s) {
      ctx.font = fs + 'px sans-serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(s.emoji, s.x * cv.width, s.y * cv.height);
      s._w = fs; s._h = fs;
    });
  };
  img.src = _lifePhotoData;
...
function editSleepEntry(id, field) {
  ...
  openModal('✏️ 修改时间', body,
    '<button class="btn btn-secondary" onclick="closeModal()">取消</button>' +
    '<button class="btn btn-primary" onclick="saveSleepEdit(' + id + ',\'' + field + '\')">保存</button>');
}

function saveSleepEdit(id, field) {
  var inp = document.getElementById('editSleepTime');
  if (!inp) return;
  ensureSleep();
  var entry = state.sleep.log.find(function (e) { return e.id === id; });
  if (!entry) return;
...
function deleteSleepEntry(id) {
  ...
  ensureSleep();
  state.sleep.log = state.sleep.log.filter(function (e) { return e.id !== id; });
  try { Store.save(); } catch (e) {}
  renderSleep();
}
