...
// 照片查看：双指捏合缩放（锚点跟随两指中心）+ 单指拖动 + 双击快速切换
function initNotePhotoZoom() {
  const img = document.getElementById('notePhotoImg');
  const scroll = document.getElementById('notePhotoScroll');
  ...
  img.addEventListener('touchstart', (e) => {
    ...
    if (e.touches.length === 2) {
      ...
      if (now - lastTap < 300) {
        ...
        else { st.scale = 2.5; st.tx = 0; st.ty = 0; }
        apply();
        lastTap = 0;
      } else { lastTap = now; }
    }
  ...
  img.addEventListener('touchmove', (e) => {
    ...
    if (e.touches.length === 2 && pinch) {
      const newScale = Math.min(5, Math.max(1, pinch.scale * (dist2(e.touches[0], e.touches[1]) / pinch.dist)));
      const o = center();
      const k = (pinch.scale - newScale) / pinch.scale;
      st.scale = newScale;
      st.tx = pinch.tx + k * (pinch.cx - o.x);
      st.ty = pinch.ty + k * (pinch.cy - o.y);
      apply();
